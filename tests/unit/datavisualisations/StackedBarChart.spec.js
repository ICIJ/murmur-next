...
describe('StackedBarChart.vue', () => {
  describe('a stacked-bars chart with two bars in four groups', () => {
    ...
    it('creates four bars', async () => {
      expect(wrapper.findAll('.stacked-bar-chart__groups__item')).toHaveLength(
        4
      )
    })
    ...
    it('creates the first group of bars with maximum width', () => {
      const firstGroup = wrapper
        .findAll('.stacked-bar-chart__groups__item')
        .at(0)
      const budgetBar = firstGroup.find(
        '.stacked-bar-chart__groups__item__bars__item--budget'
      )
      const boxOfficeBar = firstGroup.find(
        '.stacked-bar-chart__groups__item__bars__item--box-office'
      )
      const totalWidth =
        budgetBar.element.offsetWidth + boxOfficeBar.element.offsetWidth
    ...
    it('creates the second group of bars with ~27% width', () => {
      const secondGroup = wrapper
        .findAll('.stacked-bar-chart__groups__item')
        .at(1)
      const budgetBar = secondGroup.find(
        '.stacked-bar-chart__groups__item__bars__item--budget'
      )
      const boxOfficeBar = secondGroup.find(
        '.stacked-bar-chart__groups__item__bars__item--box-office'
      )
    ...
    it('creates a legend with "budget" and "box_office" items', () => {
      const budgetLegend = wrapper
        .findAll('.stacked-bar-chart__legend__item')
        .at(0)
      const boxOfficeLegend = wrapper
        .findAll('.stacked-bar-chart__legend__item')
        .at(1)
    ...
    it('highlights the legend "budget"', async () => {
      const budgetLegend = wrapper
        ...
        .at(0)
      expect(
        budgetLegend.classes('stacked-bar-chart__legend__item--highlighted')
      ).toBeFalsy()
    ...
    it('highlights the bars for "budget"', async () => {
      ...
      expect(
        budgetBars
          .at(0)
          .classes('stacked-bar-chart__groups__item__bars__item--highlighted')
      ).toBeTruthy()
      expect(
        budgetBars
          .at(1)
    ...
    it('highlights the legend "budget" on mouseover and "box_office" by default', async () => {
      ...
      expect(
        boxOfficeLegend.classes('stacked-bar-chart__legend__item--highlighted')
      ).toBeTruthy()
      await budgetLegend.trigger('mouseover')
      await vi.advanceTimersByTimeAsync(wrapper.vm.highlightDelay)
      expect(
    ...
    it('highlights the whole "Avatar" row', async () => {
      ...
      const boxOffice = wrapper
        .findAll('.stacked-bar-chart__groups__item__bars__item')
        .at(0)
    ...
    it('creates bars with specific colors', async () => {
      ...
      expect(boxOfficeBar.element.style['background-color']).toBe(
        'rgb(68, 68, 68)'
      )
    })
    ...
    it('creates legend with specific colors', async () => {
      ...
      const legendBoxes = wrapper.findAll(
        '.stacked-bar-chart__legend__item__box'
      )
    ...
    it('creates bar direct labeling without formatting', async () => {
      ...
      const values = firstGroup.findAll(
        '.stacked-bar-chart__groups__item__bars__item__value'
      )
  ...
  describe('a stacked-bars chart with 3 bars in 3 groups with empty values', () => {
    ...
    it('creates the first bar with normalized classes on bar', () => {
      const group = wrapper.findAll('.stacked-bar-chart__groups__item').at(0)
      const items = group.findAll(
        '.stacked-bar-chart__groups__item__bars__item'
      )
      expect(
        items.at(0).classes('stacked-bar-chart__groups__item__bars__item--foo')
      ).toBeTruthy()
      expect(
        items
          .at(1)
          .classes('stacked-bar-chart__groups__item__bars__item--baz-1')
      ).toBeTruthy()
      expect(
        items
          .at(2)
          .classes('stacked-bar-chart__groups__item__bars__item--baz-2')
      ).toBeTruthy()
    })
